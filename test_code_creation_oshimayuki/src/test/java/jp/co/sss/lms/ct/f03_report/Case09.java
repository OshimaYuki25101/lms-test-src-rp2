...
public class Case09 {
	...
	void test04() {
		...
		//週報レポートの「修正する」ボタンをクリックするために、下にスクロール
		scrollTo("1200");
		//「修正する」ボタンをクリック
	...
	void test09() {
		...
		//目標の達成度入力欄の取得(このメソッドでは未入力)
		WebElement achievementLevel = webDriver.findElement(By.xpath("//*[@id=\"content_0\"]"));
		achievementLevel.clear();
		achievementLevel.sendKeys("");

		//所感入力欄の取得(このメソッドでは未入力)
		WebElement impressions = webDriver.findElement(By.xpath("//*[@id=\"content_1\"]"));
		impressions.clear();
	...
	void test10() {
		...
		//所感入力欄の取得(2000文字以上を入力)
		WebElement impressions = webDriver.findElement(By.xpath("//*[@id=\"content_1\"]"));
		impressions.clear();
		impressions.sendKeys(overString);
		
		//一週間の振り返り入力欄の取得(2000文字以上を入力)
		WebElement review = webDriver.findElement(By.xpath("//*[@id=\"content_2\"]"));
		review.clear();
		review.sendKeys(overString);
